...
- **分析页**：
  ...
  - **字段拆解**：所有 select/multiselect 字段（只排掉 `result` 角色，因为按结果拆是自我循环），**外加所有 `time` 字段**（按时间段分桶，见下条）。每行 `n` 是该值下的**全部**笔数（含 BE 系列），**胜率分母只算 W 和 L**，两者口径不同是有意的。每行右边标「相对当前这批整体胜率的差值」；`n < BREAKDOWN_MIN_SAMPLE`(5) 的行不画色条、不标差值、整行降透明度并挂「样本少」标签；多选字段卡片带「多选」标记（各行 n 之和 > 总笔数是预期行为）。可勾选隐藏、拖拽排序，配置存数据库，**全局唯一一份**（不按组合分开存——那样新加字段要去每个组合里勾一遍）
