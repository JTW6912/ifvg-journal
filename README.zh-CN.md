...
- **卡片视图**，3 档图片大小（紧凑 / 标准 / 大图），可自选卡片上额外显示哪些字段，
  日期、模型、R 值固定常显。适合「在几十笔里找到那一笔」。
- **表格视图** —— 每个字段一列，每页 25 行。
- **看图视图** —— 一行一笔的大图浏览模式，适合「把这一笔看清楚」：连着翻几十笔找规律、养盘感。
  截图**不裁切**（卡片视图为了网格整齐会裁），只按高度封顶，三档高度（舒适 / 大 / 满屏）；
  字段可挂在图片右侧或底下（放底下图能吃满整行宽、更大，代价是一笔占的高度超过一屏）。
  支持 <kbd>J</kbd> / <kbd>K</kbd> 或 <kbd>↑</kbd> / <kbd>↓</kbd> 翻笔，点图看原图。
  另有**遮挡结果**开关：先盖住 result 和 R，自己判断完再点「揭晓」对答案——用来练盘感，不是查账。
...
- **编辑器** —— 底子是个普通 textarea，只是在上面堆了手感，不是块编辑器：在列表里按回车
  ...
  选区；粘贴一条图片链接会自动变成图片，选中文字再粘链接会自动变成超链接。
- **快捷键** —— `Ctrl/Cmd` 加：`B` 加粗、`I` 斜体、`E` 行内代码、`K` 链接、`S` 立刻保存；
  加 `Shift` 再加：`X` 删除线、`1`/`2`/`3` 各级标题、`8` 项目符号、`7` 编号、`9` 待办、
  `.` 引用。工具栏按钮悬停就能看到对应的键。
  **`Ctrl/Cmd+Z` 撤销和 `Ctrl/Cmd+Shift+Z` 重做走浏览器原生的那套**，工具栏、快捷键、
  回车续列表、Tab 缩进做的每一步改动都能一步步退回去。
- **`/` 插入菜单** —— 在行首打 `/` 就弹出来：标题、列表、待办、引用、代码块、分割线、表格、
  图片、链接、**关联交易**。边打边过滤，中英文都认（`/表格` 和 `/table` 都能搜到）。
- **文字颜色** —— 工具栏那个 A 按钮（或 `/颜色`）能给选中的文字上色：红、绿、黄、蓝、灰，
  外加一个高亮底色。再点一次换色、点「去掉颜色」清掉，都不会套出嵌套。源码里是
  `{red|文字}` 这种写法，颜色跟着深浅色主题走。
- **图片**用链接插入，跟交易截图一个方式。点图片走灯箱放大。
- **关联交易** —— 从选择器里挑一笔（可以按日期、模型、任意内容搜），在光标处插入一条
  `[[trade:…]]`。渲染成一枚胶囊，上面是日期、模型、结果和 R。**点一下弹出的是只读预览**
  ——不裁剪的截图加上这笔填过的所有字段，不是编辑表单；点图看原图，真要改点底部那个
  「编辑这笔交易」。看复盘时是在读，一点就弹一堆输入框既容易误改也太重。
  那笔交易以后被删了，胶囊会变红并写明「已删除的交易」，不会默默消失。
- **实时预览**（仅编辑模式）左右分栏，窄屏改成上下堆叠，也可以整个收起来。
- **两边滚动联动** —— 编辑区滚到哪一段，预览就停在同一段；光标所在的那一段在预览里用一道
  金色竖条标出来，跳到别处时预览会把它带回可视区。对齐是按「块」算的而不是按比例：正文里有
  图片或表格时两边高度差很大，按比例滚会完全对不上。自己动手滚预览时不会被打断，等你回去
  动编辑区再重新对齐。
...
管理字段结构：改名、改类型、编辑选项池（可拖拽排序）、打分析角色标签、停用字段、删除字段，
字段本身也可以整条拖拽调整顺序。改动立刻同步到录入表单和所有统计。

**停用字段（可逆）** —— 展开任意字段，里面有「停用这个字段」。停用之后：

- 录入表单里不再出现，新交易在这个字段上一律留空；
- 库里已有的数据一个字节都不动，拆解、组合、筛选、导出全都照常认这个字段；
- 编辑老交易时，那些当时填过的停用字段会折在表单底部单独一段，照样能改；
- 随时可以「恢复录入」变回来。

一套字段用久了总会有几个被拆解证明没信息量，继续每笔都填是纯成本——但**删掉**字段的话，
当初「已确认无效」的证据本身也一起没了，以后想复核那张拆解表就再也复核不了。
所以想少填几个字段一律走停用，删除只留给「这个字段从来就是建错的」。
停用带核心角色（日期 / 结果 / R值）的字段会先确认一次，因为那会让新交易的统计从下一笔起失真。
